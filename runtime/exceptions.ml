...
declare
  ...
  declare
    type t +=
      [ Help = Arg.Help[@"name" "Arg.Help";] | Bad = Arg.Bad[@"name" "Arg.Bad";] | Finally_raised = Fun.Finally_raised[@"name" "Fun.Finally_raised";] | Undefined = Lazy.Undefined[@"name" "Lazy.Undefined";] | Parse_error = Parsing.Parse_error[@"name" "Parsing.Parse_error";] | QueueEmpty = Queue.Empty[@"name" "Queue.Empty";] | Scan_failure = Scanf.Scan_failure[@"name" "Scanf.Scan_failure";] | StackEmpty = Stack.Empty[@"name" "Stack.Empty";] | Exit = Stdlib.Exit[@"name" "Stdlib.Exit";] | Match_failure = Stdlib.Match_failure[@"name" "Stdlib.Match_failure";] | Assert_failure = Stdlib.Assert_failure[@"name" "Stdlib.Assert_failure";] | Invalid_argument
        = Stdlib.Invalid_argument[@"name" "Stdlib.Invalid_argument";] | Failure = Stdlib.Failure[@"name" "Stdlib.Failure";] | Not_found = Stdlib.Not_found[@"name" "Stdlib.Not_found";] | Out_of_memory = Stdlib.Out_of_memory[@"name" "Stdlib.Out_of_memory";] | Stack_overflow = Stdlib.Stack_overflow[@"name" "Stdlib.Stack_overflow";] | Sys_error = Stdlib.Sys_error[@"name" "Stdlib.Sys_error";] | End_of_file = Stdlib.End_of_file[@"name" "Stdlib.End_of_file";] | Division_by_zero
        = Stdlib.Division_by_zero[@"name" "Stdlib.Division_by_zero";] | Sys_blocked_io = Stdlib.Sys_blocked_io[@"name" "Stdlib.Sys_blocked_io";] | Undefined_recursive_module
        = Stdlib.Undefined_recursive_module[@"name" "Stdlib.Undefined_recursive_module";] | StreamFailure = Stream.Failure[@"name" "Stream.Failure";] | Error = Stream.Error[@"name" "Stream.Error";] | Break = Sys.Break[@"name" "Sys.Break";] ][@@"deriving_inline" (show {with_path = False}, sexp, yojson, eq);]
    ;
    declare
      let open M_equal in
      let fallback = f.f in
    ...
    declare
      ...
      f.f :=
        fun
        [ Help v0 →
            `List [`String "Arg.Help"; Runtime.Yojson.string_to_yojson v0]
        | Bad v0 →
            `List [`String "Arg.Bad"; Runtime.Yojson.string_to_yojson v0]
        | Finally_raised v0 →
            `List [`String "Fun.Finally_raised"; to_yojson v0]
        | Undefined → `List [`String "Lazy.Undefined"]
        | Parse_error → `List [`String "Parsing.Parse_error"]
        | QueueEmpty → `List [`String "Queue.Empty"]
        | Scan_failure v0 →
            `List
              [`String "Scanf.Scan_failure";
        ...
        | Failure v0 →
            `List
              [`String "Stdlib.Failure"; Runtime.Yojson.string_to_yojson v0]
        ...
        | Sys_error v0 →
            `List
              [`String "Stdlib.Sys_error"; Runtime.Yojson.string_to_yojson v0]
      ...
      f.f :=
        ...
        | `List [`String "Stdlib.Division_by_zero"] →
            Result.Ok Division_by_zero
    ...
    declare
      ...
      f.f :=
        ...
        [ Help v0 →
            Sexplib0.Sexp.List
              [Sexplib0.Sexp.Atom "Arg.Help";
               Sexplib0.Sexp_conv.sexp_of_string v0]
        | Bad v0 →
            Sexplib0.Sexp.List
              [Sexplib0.Sexp.Atom "Arg.Bad";
               Sexplib0.Sexp_conv.sexp_of_string v0]
        | Finally_raised v0 →
            Sexplib0.Sexp.List
        ...
        | Failure v0 →
            Sexplib0.Sexp.List
              [Sexplib0.Sexp.Atom "Stdlib.Failure";
               Sexplib0.Sexp_conv.sexp_of_string v0]
        | Not_found →
            Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "Stdlib.Not_found"]
        | Out_of_memory →
        ...
        | Sys_error v0 →
            Sexplib0.Sexp.List
              [Sexplib0.Sexp.Atom "Stdlib.Sys_error";
               Sexplib0.Sexp_conv.sexp_of_string v0]
        | End_of_file →
            Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "Stdlib.End_of_file"]
        | Division_by_zero →
            ...
            Sexplib0.Sexp.List
              [Sexplib0.Sexp.Atom "Stdlib.Undefined_recursive_module";
               (fun (v0, v1, v2) →
                 ...
                 v0]
        | StreamFailure →
            Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "Stream.Failure"]
        | Error v0 →
            Sexplib0.Sexp.List
            ...
            Match_failure
              ((fun
                ...
                | _ → failwith "wrong number of members in list" ])
                 v0)
        | Sexplib0.Sexp.List
            [Sexplib0.Sexp.Atom "Stdlib.Assert_failure"; v0] →
            Assert_failure
              ((fun
                [ Sexplib0.Sexp.List [v0; v1; v2] →
                    (Sexplib0.Sexp_conv.string_of_sexp v0,
                ...
                | _ → failwith "wrong number of members in list" ])
                 v0)
        | Sexplib0.Sexp.List
            [Sexplib0.Sexp.Atom "Stdlib.Invalid_argument"; v0] →
    ...
    declare
      ...
      f.f :=
        fun ofmt →
          ...
          | Undefined →
              let open Runtime.Fmt in pf ofmt "@[<2>Lazy.Undefined@]"
          | Parse_error →
          ...
          | Failure v0 →
              ...
              pf ofmt "(@[<2>Stdlib.Failure@ %a)@]"
                (fun ofmt arg → let open Runtime.Fmt in pf ofmt "%S" arg) v0
          | Not_found →
              let open Runtime.Fmt in pf ofmt "@[<2>Stdlib.Not_found@]"
          | Out_of_memory →
          ...
          | Undefined_recursive_module v0 →
              ...
              pf ofmt "(@[<2>Stdlib.Undefined_recursive_module@ %a)@]"
                ...
                v0
          | StreamFailure →
              let open Runtime.Fmt in pf ofmt "@[<2>Stream.Failure@]"
          | Error v0 →
              let open Runtime.Fmt in
