...
module Records = struct
  ...
  let%expect_test _ =
    ...
    Expect_test_helpers.show_raise (fun () -> t_of_sexp sexp);
    [%expect {|
      (raised (
        Of_sexp_error
...
module Drop_default = struct
  ...
  module Equal = struct
    ...
    type t = _t = {
      a : my_int; [@default 2] [@sexp_drop_default.eq]
    } [@@deriving sexp]
  ...
  module Compare = struct
    ...
    type t = _t = {
      a : my_int; [@default 2] [@sexp_drop_default.ord]
    } [@@deriving sexp]
...
module Boolean = struct
  ...
  let%expect_test _ =
    Expect_test_helpers.require_does_raise ~cr:CR_soon
      [%here] (fun () ->
