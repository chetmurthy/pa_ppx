...
declare
    ...
    struct
      ...
      declare
        ...
        value rec pp_vala : ! α . Fmt.t α → Fmt.t (vala α) =
          fun tp_0 (ofmt : Format.formatter) arg →
            (fun ofmt →
               fun
               [ VaAnt v0 →
                   let open Fmt in
                   pf ofmt "(@[<2>Pp_MLast.Ploc.VaAnt@ %a)@]"
                     (fun ofmt arg → let open Fmt in pf ofmt "%S" arg) v0
               | VaVal v0 →
                   let open Fmt in
        ...
        and show_vala : ! α . Fmt.t α → vala α → Stdlib.String.t =
          fun tp_0 arg → Format.asprintf "%a" (pp_vala tp_0) arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
        ;
        [@@@"end"];
      end;
    end
             ...
             (Ploc.pp_vala
                (fun ofmt →
                   ...
                   | Some arg →
                       let open Fmt in
                       pf ofmt "(Some %a)"
                         (fun ofmt arg → let open Fmt in pf ofmt "%S" arg)
                         arg ]))
             v0
             (fun ofmt →
                fun
                [ None → let open Fmt in const string "None" ofmt ()
                | Some arg →
                    let open Fmt in pf ofmt "(Some %a)" Fmt.bool arg ])
             v1)
          ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
  ...
  declare
    ...
    and generic_constructor =
      (loc * Ploc.vala string * Ploc.vala (list ctyp) * option ctyp *
       attributes)
    ...
    value rec pp_class_infos : ! α . Fmt.t α → Fmt.t (class_infos α) =
      fun tp_0 (ofmt : Format.formatter) arg →
        (fun ofmt
             {ciLoc = v_ciLoc; ciVir = v_ciVir; ciPrm = v_ciPrm;
              ciNam = v_ciNam; ciExp = v_ciExp;
              ciAttributes = v_ciAttributes} →
           let open Fmt in
           pf ofmt
                ...
                pf ofmt "(@[%a,@ %a@])" pp_loc v0
                  (Ploc.pp_vala
                     (fun (ofmt : Format.formatter) arg →
                        let open Fmt in
                        pf ofmt "@[<2>[%a@,]@]"
                          (list ~{sep = semi} pp_type_var) arg))
                  v1)
             v_ciPrm
             (Ploc.pp_vala (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
             v_ciNam tp_0 v_ciExp pp_attributes v_ciAttributes)
          ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    and show_class_infos : ! α . Fmt.t α → class_infos α → Stdlib.String.t =
      fun tp_0 arg → Format.asprintf "%a" (pp_class_infos tp_0) arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    ...
    and pp_ctyp : Fmt.t ctyp =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           | TyAny v0 →
               let open Fmt in pf ofmt "(@[<2>MLast.TyAny@ %a)@]" pp_loc v0
           | TyApp v0 v1 v2 →
               let open Fmt in
           ...
           | TyMan v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.TyMan@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_ctyp v1 (Ploc.pp_vala Fmt.bool) v2 pp_ctyp v3
           | TyObj v0 v1 v2 →
               let open Fmt in
           ...
           | TyOlb v0 v1 v2 →
               ...
               pf ofmt "(@[<2>MLast.TyOlb@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 v1 pp_ctyp v2
           | TyOpn v0 →
               let open Fmt in pf ofmt "(@[<2>MLast.TyOpn@ %a)@]" pp_loc v0
           | TyPck v0 v1 →
               let open Fmt in
               ...
               pf ofmt "(@[<2>MLast.TyRec@ (@,%a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       ...
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi}
                            (fun (ofmt : Format.formatter)
                                 (v0, v1, v2, v3, v4) →
                               let open Fmt in
                               pf ofmt "(@[%a,@ %a,@ %a,@ %a,@ %a@])" pp_loc
                                 v0
                                 (fun ofmt arg →
                                    let open Fmt in pf ofmt "%S" arg)
           ...
           | TyTup v0 v1 →
               ...
               pf ofmt "(@[<2>MLast.TyTup@ (@,%a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_ctyp)
                         arg))
                 v1
           | TyVrn v0 v1 v2 →
               let open Fmt in
                    ...
                    | Some arg →
                        ...
                        pf ofmt "(Some %a)"
                          (fun ofmt →
                             fun
                             [ None →
                                 let open Fmt in const string "None" ofmt ()
                             | Some arg →
                                 let open Fmt in
                                 pf ofmt "(Some %a)"
                                   (Ploc.pp_vala
           ...
           | TyXtr v0 v1 v2 →
               ...
               pf ofmt "(@[<2>MLast.TyXtr@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 (fun ofmt arg → let open Fmt in pf ofmt "%S" arg) v1
                 (fun ofmt →
                    fun
                    [ None → let open Fmt in const string "None" ofmt ()
                    | Some arg →
      ...
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           [ PvTag v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.PvTag@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 (Ploc.pp_vala Fmt.bool) v2
    ...
    and pp_patt : Fmt.t patt =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           | PaLong v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.PaLong@ (@,%a,@ %a@,))@]" pp_loc v0
                 pp_longid v1
           | PaAli v0 v1 v2 →
               let open Fmt in
           ...
           | PaAnt v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.PaAnt@ (@,%a,@ %a@,))@]" pp_loc v0 pp_patt
                 v1
           | PaAny v0 →
               let open Fmt in pf ofmt "(@[<2>MLast.PaAny@ %a)@]" pp_loc v0
           | PaApp v0 v1 v2 →
               let open Fmt in
           ...
           | PaArr v0 v1 →
               ...
               pf ofmt "(@[<2>MLast.PaArr@ (@,%a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_patt)
                         arg))
                 v1
           | PaChr v0 v1 →
               let open Fmt in
           ...
           | PaExc v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.PaExc@ (@,%a,@ %a@,))@]" pp_loc v0 pp_patt
                 v1
           | PaFlo v0 v1 →
               let open Fmt in
           ...
           | PaInt v0 v1 v2 →
               ...
               pf ofmt "(@[<2>MLast.PaInt@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 (fun ofmt arg → let open Fmt in pf ofmt "%S" arg) v2
           | PaLab v0 v1 →
               let open Fmt in
           ...
           | PaLaz v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.PaLaz@ (@,%a,@ %a@,))@]" pp_loc v0 pp_patt
                 v1
           | PaLid v0 v1 →
               let open Fmt in
               ...
               pf ofmt "(@[<2>MLast.PaOlb@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 (Ploc.pp_vala
                    (fun ofmt →
                       ...
                       [ None → let open Fmt in const string "None" ofmt ()
                       | Some arg →
                           let open Fmt in pf ofmt "(Some %a)" pp_expr arg ]))
                 v2
           | PaOrp v0 v1 v2 →
               let open Fmt in
    ...
    and pp_expr : Fmt.t expr =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           | ExAnt v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExAnt@ (@,%a,@ %a@,))@]" pp_loc v0 pp_expr
                 v1
           | ExApp v0 v1 v2 →
               let open Fmt in
           ...
           | ExAre v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExAre@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 pp_expr v2
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]" (list ~{sep = semi} pp_expr)
                         arg))
                 v3
           | ExArr v0 v1 →
               let open Fmt in
           ...
           | ExAsr v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExAsr@ (@,%a,@ %a@,))@]" pp_loc v0 pp_expr
                 v1
           | ExAss v0 v1 v2 →
               let open Fmt in
           ...
           | ExBae v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExBae@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 pp_expr v2
           ...
           | ExCoe v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExCoe@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_expr v1
                 (fun ofmt →
                    fun
                    [ None → let open Fmt in const string "None" ofmt ()
                    | Some arg →
                        let open Fmt in pf ofmt "(Some %a)" pp_ctyp arg ])
                 v2 pp_ctyp v3
           | ExFlo v0 v1 →
               let open Fmt in
               ...
               pf ofmt "(@[<2>MLast.ExFun@ (@,%a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi} pp_case_branch) arg))
                 v1
           | ExIfe v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExIfe@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_expr v1 pp_expr v2 pp_expr v3
           | ExInt v0 v1 v2 →
               let open Fmt in
           ...
           | ExLaz v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExLaz@ (@,%a,@ %a@,))@]" pp_loc v0 pp_expr
                 v1
           | ExLet v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExLet@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                    ...
                    (fun (ofmt : Format.formatter) arg →
                       ...
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi}
                            (fun (ofmt : Format.formatter) (v0, v1, v2) →
                               let open Fmt in
                               pf ofmt "(@[%a,@ %a,@ %a@])" pp_patt v0 pp_expr
                                 v1 pp_attributes v2))
                         arg))
                 v2 pp_expr v3
           | ExLEx v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExLEx@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1
           ...
           | ExLmd v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExLmd@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0
                 (Ploc.pp_vala
                    (fun ofmt →
                       fun
           ...
           | ExLop v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExLop@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 (Ploc.pp_vala Fmt.bool) v1 pp_module_expr v2 pp_expr v3
           | ExMat v0 v1 v2 →
               let open Fmt in
               ...
               pf ofmt "(@[<2>MLast.ExObj@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt →
                       ...
                       [ None → let open Fmt in const string "None" ofmt ()
                       | Some arg →
                           let open Fmt in pf ofmt "(Some %a)" pp_patt arg ]))
                 v1
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
           ...
           | ExSte v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.ExSte@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 pp_expr v2
           ...
           | ExExten v0 v1 →
               ...
               pf ofmt "(@[<2>MLast.ExExten@ (@,%a,@ %a@,))@]" pp_loc v0
                 pp_attribute v1
           | ExUnr v0 →
               let open Fmt in pf ofmt "(@[<2>MLast.ExUnr@ %a)@]" pp_loc v0 ])
          ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    ...
    and pp_module_type : Fmt.t module_type =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           [ MtLong v0 v1 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.MtLong@ (@,%a,@ %a@,))@]" pp_loc v0
                 pp_longid v1
           | MtLongLid v0 v1 v2 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.MtLongLid@ (@,%a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_longid v1
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v2
               ...
               pf ofmt "(@[<2>MLast.MtWit@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi} pp_with_constr) arg))
                 v2
           | MtXtr v0 v1 v2 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.MtXtr@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 (fun ofmt →
                    ...
                    | Some arg →
                        let open Fmt in
                        pf ofmt "(Some %a)" (Ploc.pp_vala pp_module_type)
                          arg ])
                 v2
           | MtAtt v0 v1 v2 →
               let open Fmt in
      ...
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
               ...
               pf ofmt "(@[<2>MLast.SgCls@ (@,%a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi} (pp_class_infos pp_class_type))
                         arg))
                 v1
           | SgClt v0 v1 →
               let open Fmt in
           ...
           | SgExt v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.SgExt@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 pp_ctyp v2
           ...
           | SgMty v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.SgMty@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
           ...
           | SgMtyAlias v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.SgMtyAlias@ (@,%a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
           ...
           | SgModSubst v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.SgModSubst@ (@,%a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
               ...
               pf ofmt "(@[<2>MLast.SgTyp@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi} pp_type_decl) arg))
                 v2
           | SgTypExten v0 v1 →
               let open Fmt in
                    ...
                    (fun (ofmt : Format.formatter) arg →
                       ...
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi}
                            (fun (ofmt : Format.formatter) (v0, v1) →
                               let open Fmt in
                               pf ofmt "(@[%a,@ %a@])" pp_sig_item v0 pp_loc
                                 v1))
                         arg))
                 v2
           | SgVal v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.SgVal@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
      ...
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           | WcTyp v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.WcTyp@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0 (Ploc.pp_vala pp_longid_lident) v1
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]"
           ...
           | WcTys v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.WcTys@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
    ...
    and pp_module_expr : Fmt.t module_expr =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           | MeUnp v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.MeUnp@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_expr v1
                 (fun ofmt →
                    fun
                    [ None → let open Fmt in const string "None" ofmt ()
           ...
           | MeXtr v0 v1 v2 →
               ...
               pf ofmt "(@[<2>MLast.MeXtr@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 (fun ofmt →
                    ...
                    | Some arg →
                        let open Fmt in
                        pf ofmt "(Some %a)" (Ploc.pp_vala pp_module_expr)
                          arg ])
                 v2
           | MeAtt v0 v1 v2 →
               let open Fmt in
      ...
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
               ...
               pf ofmt "(@[<2>MLast.StCls@ (@,%a,@ %a@,))@]" pp_loc v0
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi} (pp_class_infos pp_class_expr))
                         arg))
                 v1
           | StClt v0 v1 →
               let open Fmt in
           ...
           | StExt v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.StExt@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 pp_ctyp v2
           ...
           | StMty v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.StMty@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v1 pp_module_type v2 pp_attributes v3
           ...
           | StOpn v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.StOpn@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 (Ploc.pp_vala Fmt.bool) v1 pp_module_expr v2 pp_attributes
                 v3
           | StTyp v0 v1 v2 →
               let open Fmt in
                    ...
                    (fun (ofmt : Format.formatter) arg →
                       ...
                       pf ofmt "@[<2>[%a@,]@]"
                         (list ~{sep = semi}
                            (fun (ofmt : Format.formatter) (v0, v1) →
                               let open Fmt in
                               pf ofmt "(@[%a,@ %a@])" pp_str_item v0 pp_loc
                                 v1))
                         arg))
                 v2
           | StVal v0 v1 v2 →
    ...
    and pp_type_extension : Fmt.t type_extension =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt
             {teNam = v_teNam; tePrm = v_tePrm; tePrv = v_tePrv;
              teECs = v_teECs; teAttributes = v_teAttributes} →
           let open Fmt in
           pf ofmt
    ...
    and pp_class_type : Fmt.t class_type =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           [ CtLongLid v0 v1 v2 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CtLongLid@ (@,%a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_longid v1
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v2
           ...
           | CtLop v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CtLop@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 (Ploc.pp_vala Fmt.bool) v1 pp_longid v2 pp_class_type v3
           | CtCon v0 v1 v2 →
               let open Fmt in
           ...
           | CtXtr v0 v1 v2 →
               ...
               pf ofmt "(@[<2>MLast.CtXtr@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 (fun ofmt →
                    ...
                    | Some arg →
                        let open Fmt in
                        pf ofmt "(Some %a)" (Ploc.pp_vala pp_class_type)
                          arg ])
                 v2
           | CtAtt v0 v1 v2 →
               let open Fmt in
    ...
    and pp_class_sig_item : Fmt.t class_sig_item =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           [ CgCtr v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CgCtr@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_ctyp v1 pp_ctyp v2 pp_attributes v3
           | CgDcl v0 v1 →
               let open Fmt in
           ...
           | CgMth v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CgMth@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0 (Ploc.pp_vala Fmt.bool) v1
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
           ...
           | CgVal v0 v1 v2 v3 v4 v5 →
               let open Fmt in
               pf ofmt
                 "(@[<2>MLast.CgVal@ (@,%a,@ %a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0 (Ploc.pp_vala Fmt.bool) v1 (Ploc.pp_vala Fmt.bool)
                 v2
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
           ...
           | CgVir v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CgVir@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0 (Ploc.pp_vala Fmt.bool) v1
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v2 pp_ctyp v3 pp_attributes v4
    ...
    and pp_class_expr : Fmt.t class_expr =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           | CeLet v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CeLet@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 (Ploc.pp_vala Fmt.bool) v1
                 (Ploc.pp_vala
                    (fun (ofmt : Format.formatter) arg →
                       let open Fmt in
           ...
           | CeLop v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CeLop@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 (Ploc.pp_vala Fmt.bool) v1 pp_longid v2 pp_class_expr v3
           | CeStr v0 v1 v2 →
               let open Fmt in
           ...
           | CeXtr v0 v1 v2 →
               ...
               pf ofmt "(@[<2>MLast.CeXtr@ (@,%a,@ %a,@ %a@,))@]" pp_loc v0
                 ...
                 (fun ofmt →
                    ...
                    | Some arg →
                        let open Fmt in
                        pf ofmt "(Some %a)" (Ploc.pp_vala pp_class_expr)
                          arg ])
                 v2
           | CeAtt v0 v1 v2 →
               let open Fmt in
    ...
    and pp_class_str_item : Fmt.t class_str_item =
      fun (ofmt : Format.formatter) arg →
        (fun ofmt →
           ...
           [ CrCtr v0 v1 v2 v3 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CrCtr@ (@,%a,@ %a,@ %a,@ %a@,))@]" pp_loc
                 v0 pp_ctyp v1 pp_ctyp v2 pp_attributes v3
           | CrDcl v0 v1 →
               let open Fmt in
           ...
           | CrInh v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CrInh@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0 (Ploc.pp_vala Fmt.bool) v1 pp_class_expr v2
                 (Ploc.pp_vala
                    (fun ofmt →
                       fun
                       [ None → let open Fmt in const string "None" ofmt ()
           ...
           | CrVav v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CrVav@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0 (Ploc.pp_vala Fmt.bool) v1
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v2 pp_ctyp v3 pp_attributes v4
           | CrVir v0 v1 v2 v3 v4 →
               let open Fmt in
               pf ofmt "(@[<2>MLast.CrVir@ (@,%a,@ %a,@ %a,@ %a,@ %a@,))@]"
                 pp_loc v0 (Ploc.pp_vala Fmt.bool) v1
                 (Ploc.pp_vala
                    (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
                 v2 pp_ctyp v3 pp_attributes v4
        ...
        (fun (ofmt : Format.formatter) (v0, v1) →
           ...
           pf ofmt "(@[%a,@ %a@])"
             (fun ofmt →
                ...
                [ None → let open Fmt in const string "None" ofmt ()
                | Some arg →
                    let open Fmt in pf ofmt "(Some %a)" pp_longid arg ])
             v0
             (Ploc.pp_vala (fun ofmt arg → let open Fmt in pf ofmt "%S" arg))
             v1)
          ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
    ...
    and pp_attribute : Fmt.t attribute =
      fun (ofmt : Format.formatter) arg →
        Ploc.pp_vala pp_attribute_body ofmt arg[@@"ocaml.warning" "-39";] [@@"ocaml.warning" "-33";]
